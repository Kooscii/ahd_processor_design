...
asm_code = open('../testbench/asm_code/%s.asm'%code_name, 'r').readlines()
...
# print the code
with open('../testbench/asm_code/rc5_optimized.code', 'w') as f:
    for no, inst in enumerate(instructions):
        if no in addr2labl:
            print(*addr2labl[no])
...
with open('../testbench/asm_code/%s.binary'%code_name, 'w') as f:
    for no, _ in enumerate(instructions):
        print(no, ':', inst_bin[no])
        f.write(inst_bin[no]+'\n')

with open('../testbench/asm_code/%s.hex'%code_name, 'w') as f:
    for no, _ in enumerate(instructions):
        print(inst_hex[no])
        f.write(inst_hex[no]+'\n')
